...
#### **Warnings: ** (if empty then none present)
```{r, tidy=TRUE, echo=FALSE, warning=FALSE, message=FALSE,tidy.opts=list(width.cutoff= I(60))}
...
dup_warning <- validate_that(sum(duplicated(genelist)) == 0,
 msg = paste("WARNING: ", sum(duplicated(genelist)), " duplicate(s) found in supplied gene list file '",
   gene_list_file,
   sep = ""
 )
)
...
## **Analysing data for:  **  
```{r, echo=FALSE}
# get unique gene types and treatment groups and constructs
unique_gene_types <- paste(unlist(unique(keep_data_info$GeneType)), collapse = ", ")
unique_treatment_groups <- paste(unlist(unique(sample_annot$Treatment)), collapse = ", ")
unique_constructs <-paste(unlist(unique(colnames(keep_data[,2:length(colnames(keep_data))]))), collapse = ", ")

# present these to user
paste(length(unique(keep_data_info$GeneType)), "unique gene types: ", unique_gene_types)
paste(length(unique(sample_annot$Treatment)), "unique treatment groups: ",unique_treatment_groups)
paste(length(unique(colnames(keep_data[,2:length(colnames(keep_data))]))), "unique constructs: ",unique_constructs)

# present total number of unique genes supplied to user
paste(length(unique(keep_data_info$LongName)), "unique genes from", gene_list_file)
...
# data are transformed into a matrix and transformed on a Log2 scale
data_matrix <- as.matrix(data)
data_matrix[, 2:length(colnames(data_matrix))] <- 
  log2(data_matrix[, 2:length(colnames(data_matrix))] + 1)
```

```{r, echo=FALSE}
knitr::kable(head(data_matrix, 5))
```

```{r, echo=TRUE, eval= FALSE}
# only data for genes from the genelist are kept
keep_data <- data_matrix[genelist, ]

# gene information is added from the gene_annot_file
keep_data_info <- merge(keep_data, gene_annot, by = "X")

# column names and rownames are improved
rownames(keep_data_info) <- keep_data_info$LongName
names(keep_data_info)[names(keep_data_info) == "Type"] <- "GeneType"
```

```{r, echo=FALSE, eval=TRUE}
knitr::kable(head(keep_data_info[, c("Gene","GeneType")], 5))
```
